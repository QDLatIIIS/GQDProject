...
def make_system(r=20, a=1.42, t=3.0, dis_z=1.0, t2 = 0.4):
	...
	def circle(pos):
		x, y, z = pos
		rsq = x**2 + y**2
		return rsq < r**2 and -0.7*dis_z < z < 1.4*dis_z
	...
	def hopB(site1, site2, B=0, U=1):
		...
		"""The constant 0.0000151927 is due to units(length in 10^-10 m)
		   Doesn't work
		"""
		return	-t *exp(-1j *0.0000151927* B * ymid * dx)
	...
	def interLayerHopping():
		sites = sys.sites()
		for site1 in sites:
			if site1.family == a1 and a1.pos(site1.tag)[2] == dis_z:
				for site2 in sites:
...
def plot_system(sys, lat):
    ...
    kwant.plot(sys, site_size = 0.25, site_lw=0.1, site_color=family_color, hop_lw=0.1)
...
def main():
	sys, BLG = make_system(r=20, a=2.46, dis_z=2)
	...
	try:
		...
		plot_spectrum(sys.finalized(), [iB * 0.02 for iB in xrange(500)], numOfEnLevel=30)
		print "Done."
